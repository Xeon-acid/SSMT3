...
public:
	...
	D3dxIniUtils(std::wstring d3dxIniFilePath) {
		if (!std::filesystem::exists(d3dxIniFilePath)) {
			parse_error = L"Your 3Dmigoto folder's d3dx.ini didn't exists, please install or update your 3Dmigoto package.\n";
			return;
		}
		...
		if (!buf) {
			parse_error = L"Can't allocate buffer for d3dx.ini\n";
			return;
		}
